 ...
 * the field directly, and we have to use the function @access_hdr@ or
 * @access_pld@.  We cannot do @P::access_hdr(p)@, because @P@ may be
 * a pointer. However, we can always rely on @packet_trait@ to strip
 * of the pointer, so we have
 * @packet_trait<P>::nonpointer_t::access_hdr(p)@.  This expression
 * looks ugly, but if we can define beforehand the nonpointer type as, say,
 * @P_T@, by doing '@typedef@ @packet_trait<P>::nonpointer_t@ @P_T@;',
 * then the original expression can be simplified as
...
/************************************************************************
 * @<h2>coordinate3d_t</h2>@
 * 
 * This class defines the coordination of a position in a three dimensional space.
 ************************************************************************/
